...
class AgentToolCardEmitTest {
    ...
    private static ToolExecutionResult result(String id, String name, String text, boolean timedOut) {
        return new ToolExecutionResult(id, name, "{}", text, 5L, timedOut, List.of());
    }

    @Test
    void nonCommandToolEmitsOutputAndOkResult() {
        CapturingRenderer r = new CapturingRenderer();
        Agent.emitToolCardResult(r, result("c1", "web_search", "搜索结果…", false));
    ...
    void executeCommandIsSkippedEntirely() {
        ...
        CapturingRenderer r = new CapturingRenderer();
        Agent.emitToolCardResult(r, result("c2", "execute_command", "hi", false));
    ...
    void timedOutToolEmitsNotOk() {
        CapturingRenderer r = new CapturingRenderer();
        Agent.emitToolCardResult(r, result("c3", "read_file", "工具执行超时（60秒），已取消", true));
    ...
    void failedPrefixEmitsNotOk() {
        CapturingRenderer r = new CapturingRenderer();
        Agent.emitToolCardResult(r, result("c4", "read_file", "工具执行失败: boom", false));
    ...
    void emptyResultEmitsOnlyResultNoOutput() {
        CapturingRenderer r = new CapturingRenderer();
        Agent.emitToolCardResult(r, result("c5", "todo_write", "", false));
        assertTrue(r.outputs.isEmpty());
    ...
    void oversizedResultIsTruncated() {
        ...
        Agent.emitToolCardResult(r, result("c6", "read_file", big, false));
        assertEquals(1, r.outputs.size());
    ...
    void nullRendererIsSafeNoop() {
        assertDoesNotThrow(() -> Agent.emitToolCardResult(null, result("c7", "read_file", "x", false)));
    }
}
