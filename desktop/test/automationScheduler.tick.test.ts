...
describe('scheduler tick 集成(C1 宽限窗)', () => {
  it('daily 14:30 模拟 24h:恰好触发 1 次', () => {
    ...
    // 从今天 08:00+PHASE 起步进 24h(覆盖今天 14:30 与明天 14:30 之间的窗口)
    const start = at(2026, 7, 2, 8, 0) + PHASE
  ...
  it('weekly 任务模拟 168h:恰好触发 1 次', () => {
    ...
    // 从周四 08:00+PHASE 起步进 7 天(覆盖本周四 09:00 与下周四 09:00 之间)
    const start = at(2026, 7, 2, 8, 0) + PHASE
  ...
  it('重启空档:lastFiredAt=null、enabledAt=三天前,从今天 15:00 起(时刻已过+超宽限)→ 今天不触发,明天 14:30 触发 1 次(不补跑)', () => {
    ...
    // 从今天 15:00+PHASE 起步进到明天 15:00(约 24h)
    const start = at(2026, 7, 2, 15, 0) + PHASE
    const { fires, fireTimes } = simulate(t, start, DAY)
  ...
  it('interval 30min 模拟 4h:触发 8 次', () => {
    ...
    // 锚点 = enabledAt(10:00);首次到点 10:30。从 10:00+PHASE 步进 4h → 10:30,11:00,...,14:00 共 8 次
    const start = at(2026, 7, 2, 10, 0) + PHASE
