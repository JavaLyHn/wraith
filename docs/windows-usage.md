...
左侧栏的**运行 → 活动**会汇总当前桌面端已知的会话、后台任务和自动化运行，并可跨项目查看；入口始终保留，即使当前没有运行中的项目也可以查看最近结果。

| 分组 | 含义 |
|---|---|
| **正在运行** | 正在执行的会话、任务或自动化 |
| **等待处理** | 正在等待审批、选择或计划审阅的工作 |
| **最近结果** | 已完成、失败、已取消或已中断的最近记录（最多显示 10 条） |

点每条记录的**打开会话 / 查看任务 / 查看自动化**可进入对应的既有页面。运行中或等待中的会话可在这里点**取消**：它只会中断当前正在打开的会话；后台任务则请求取消该任务。自动化运行暂不支持从活动中心停止，需到自动化页面处理。取消请求成功只表示已交给后端，列表会在后续状态更新后显示最终结果。

如果后端断开连接，活动中心会保留最后一份快照并标记**数据可能已过期**；此时运行中的会话会显示为已中断，等待中的会话会变为未知。桌面端重启后，活动中心的进程内记录会从新的实时事件和快照重新建立，因此后端重新连接、刷新成功前的数据可能不完整，不能据此判断实际运行状态。请先恢复连接后点**刷新**。
