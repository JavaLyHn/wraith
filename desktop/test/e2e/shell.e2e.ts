...
test('happy path: submit turn, see markdown+thinking+tool+approval, approve, see output', async () => {
  ...
  // 5. 工具成功完成:execute_command 成功显示「✓ 完成」徽标
  //    (exit 码仅在失败时出现,见 shared/toolBadge.ts 的 toolBadgeLabel)
  await expect(
    win.locator('[data-testid="tool-card"]')
  ).toContainText('✓ 完成', { timeout: 10000 })

  // 6. 成功卡默认折叠(toolCardDefaultExpanded:仅失败展开);展开后 stdout 'hi' 可见
  await win.locator('[data-testid="tool-card-header"]').click()
  await expect(
...
test('reconnect after restart re-resumes the active session', async () => {
  ...
  await expect(approveBtn).toBeVisible({ timeout: 10000 })
  await approveBtn.click()
  await expect(win.locator('[data-testid="tool-card"]')).toContainText('✓ 完成', { timeout: 15000 })
...
test('approval 后 transcript 出现 diff 卡片(文件名可见)', async () => {
  ...
  await expect(win.locator('[data-testid="file-artifact-card"]')).toBeVisible({ timeout: 15000 })
  await expect(win.locator('[data-testid="file-artifact-card"]')).toContainText('hello.txt', { timeout: 10000 })
...
test('长对话溢出后 tool/thinking/diff 卡片保持完整高度(不被压成 2px 线)', async () => {
  ...
  for (let turn = 1; turn <= 4; turn++) {
    ...
    await expect(approveBtn).toBeVisible({ timeout: 10000 })
    await approveBtn.click()
    await expect(win.locator('[data-testid="file-artifact-card"]')).toHaveCount(turn, { timeout: 10000 })
  }
  ...
  for (const testid of ['tool-card', 'thinking', 'file-artifact-card']) {
...
test('编辑用户消息 → session.rewind + 新文本重发,气泡更新', async () => {
  ...
  await expect(win.locator('[data-testid="approve"]')).toBeVisible({ timeout: 10000 })
  await win.locator('[data-testid="approve"]').click()
  await expect(win.locator('[data-testid="tool-card"]').first()).toContainText('✓ 完成', { timeout: 15000 })
...
test('长对话发送后自动滚到底部;上翻后再次发送强制回底', async () => {
  ...
  await expect(win.locator('[data-testid="approve"]')).toBeVisible({ timeout: 10000 })
  await win.locator('[data-testid="approve"]').click()
  await expect(win.locator('[data-testid="file-artifact-card"]')).toHaveCount(5, { timeout: 10000 })
  await expect.poll(atBottom, { timeout: 5000 }).toBe(true)
...
test('T44 模型切换:开下拉→选 deepseek→chip+record session.setModel;未配置 provider 隐藏', async () => {
  ...
  // 切换下拉只列【已配置(hasKey)】的 provider(configuredProviders 有意过滤未配置项,
  // 见 lib/modelSwitcher.ts)。mock 里 deepseek 有 key、openai 无 key → 仅 deepseek 出现。
  const options = win.locator('[data-testid="model-option"]')
  await expect(options).toHaveCount(1, { timeout: 10000 })

  // 无 key 的 openai 不再以「置灰项」出现,而是整条隐藏(旧「显示为 disabled」行为已移除)
  await expect(options.filter({ hasText: 'openai' })).toHaveCount(0)
...
test('T45 设为默认:点 model-set-default → record config.setDefaultProvider', async () => {
  ...
  // 等条目出现;切换下拉只列已配置 provider → 仅 deepseek(openai 无 key 隐藏)。
  // deepseek 有 key 且非默认(mock 默认为 openai)→ 会渲染「设为默认」按钮。
  const options = win.locator('[data-testid="model-option"]')
  await expect(options).toHaveCount(1, { timeout: 10000 })
...
// ---------------------------------------------------------------------------
// T46: 命令面板搜索 — 侧栏内联搜索已改为 CommandPalette(nav-search 打开),
//   面板内仍按关键字过滤「会话 + 项目」两分区;清空查询恢复全部结果。
//   (旧 sidebar-search / search-project-item / sidebar-search-clear 交互已下线。)
// ---------------------------------------------------------------------------

test('T46 命令面板搜索:输入关键字过滤会话+项目,清空恢复全部', async () => {
  ...
  // nav-search 打开命令面板,聚焦搜索输入
  await win.locator('[data-testid="nav-search"]').click()
  await expect(win.locator('[data-testid="command-palette"]')).toBeVisible({ timeout: 5000 })
  const palInput = win.locator('[data-testid="palette-input"]')
  await expect(palInput).toBeVisible()

  // 输入 "alpha":命中「alpha 测试会话」(会话分区)+ 路径尾段含 alpha 的 dirB(项目分区);
  // 「无关对话」与其余固定命令/导航项均不含 alpha,被过滤 → 结果恰为 1 会话 + 1 项目。
  await palInput.fill('alpha')
  await expect(win.locator('[data-testid="palette-item"]')).toHaveCount(2, { timeout: 5000 })
  await expect(win.locator('[data-testid="palette-item"]').filter({ hasText: 'alpha 测试会话' })).toHaveCount(1)
  await expect(win.locator('[data-testid="palette-item"]').filter({ hasText: '无关对话' })).toHaveCount(0)

  // 清空查询 → 「无关对话」重新出现在结果中(空 query 返回全部会话+项目+命令)
  await palInput.fill('')
  await expect(win.locator('[data-testid="palette-item"]').filter({ hasText: '无关对话' })).toHaveCount(1, { timeout: 5000 })
