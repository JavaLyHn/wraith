...
- **并发互斥**:治理动作与 ReAct 主循环互斥——`compactNow` 在后端判定"回合运行中"直接拒绝(返回 busy 错误给前端 notice),不再只靠前端按钮禁用;`curate` 只在主循环线程调 LLM 前执行,天然串行。history 的一切改写只发生在这两个入口。
- **范围**:本设计只治理主 `Agent` 的 conversationHistory;`SubAgent`/`PlanExecuteAgent` 各自的 history 本期不接(子代理短命,爆窗风险低;后续接入只是复用 curator 实例,预留但不做)。
- **滞回目标口径**:批量释放后没有新真实 usage,目标线判定用 `上次真实 inputTokens − Σ(本次各变更的估算释放量)`;下一次真实 usage 到来即校准。估算只用于"释放了多少"的相对量,触发判断始终以真实值为锚。
...
- 活摘要预览取数:新 RPC `context.state.get` 返回 `{watermark, liveSummary, protectedRounds, stats 累计}`——面板打开时拉一次,之后靠事件增量刷新。
- `savedTokens` 口径:Tier1/2 的节省是估算值(真实节省体现在下一轮 usage),面板如实标注"估算";Tier3 的 before/after 用前后两轮真实 usage 校准后回填。
