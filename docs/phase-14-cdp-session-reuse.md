...
>
> **核心原则**：第 13 期已经把 `chrome-devtools-mcp` 接进来跑通；本期**不写 CDP 协议**——chrome-devtools-mcp 原生支持 `--autoConnect` 连接 Chrome 144+ 的 `chrome://inspect/#remote-debugging` 实例，也支持旧式 `--browser-url=http://127.0.0.1:9222`，Wraith 这边只做「运行时切换 server 启动参数 + 登录态安全约束 + UX 引导」。
...
- `close_page` 工具硬保护：只允许关闭 Wraith 当次会话 `new_page` 出来的 tab，否则抛 `PolicyException`
...
- 跨设备 / 跨 Wraith 实例的 session 同步
- Cookie 持久化到 Wraith 自己的存储
...
**默认行为不变**：`~/.wraith/mcp.json` 不存在时仍生成 `--isolated=true`（向后兼容第 13 期用户）。`browser_connect` / `/browser connect` 是运行期动作，不主动改用户 mcp.json 文件，**只在内存中切换 args**；用户重启 Wraith 后回到 isolated。这样：
- 默认安全（isolated 仍是默认）
- 运行时按需 connect
- 用户的 mcp.json 不被 Wraith 偷偷改动
...
- `close_page` 调用前 `BrowserGuard.check()` 校验：目标 pageId 不在 `agentOpenedTabs` 里 → 抛 `PolicyException("close_page 拒绝：该 tab 不是 Wraith 本次会话开启的，不允许关闭。如需关闭请用户手动操作。")`
...
**isolated 模式下的行为**：tabs 都是 Wraith 自己开的，正常允许 close。判定：
- isolated 模式 → 跳过该硬保护
- shared 模式 → 启用
...
**启动期**：Wraith 启动时，如果 `mcp.json` 里 `chrome-devtools` 配置已经是 `--browser-url=...`（用户自己改的），不在启动期探活——`chrome-devtools-mcp` 自己会失败并写 stderr，`/mcp logs chrome-devtools` 可见。启动期间不阻塞别的 server。
...
`~/.wraith/sensitive_patterns.txt`（**Wraith 不主动创建**，只读取）：
...
- Wraith 已对敏感 URL（settings / admin / billing / account / security / oauth 等）启用强制单步审批，每个改写型工具调用都会弹窗
...
🛡️ 策略拒绝: close_page 拒绝：该 tab 不是 Wraith 本次会话开启的，
   不允许关闭。如需关闭请用户手动操作。

[Agent 转告用户]
我无法关闭你原本打开的 GitHub tab，Wraith 只允许关闭它自己开的 tab。
你可以在 Chrome 里直接关掉，或者告诉我关 Wraith 自己开过的哪个 tab。
```
...
```
> /browser connect
[/browser tabs 显示用户已开 7 个 tab，Wraith 自开 0 个]
> 帮我关掉第一个 tab
```
...
3. **`close_page` 的 pageId 解析**：chrome-devtools-mcp 工具返回结构 Wraith 这边没强约定，需要在 Day 1 实际跑一次 `new_page` 看看 result 长什么样，再决定怎么解析 pageId。如果发现完全没法解析，**保守**：shared 模式下一律拒绝 `close_page`，让用户手动关。
4. **敏感规则的 URL 来源**：`navigate_page` 的 args 显式带 url，好办；但 `click` / `fill` 这些 args 里没 url，靠 `BrowserSession.lastNavigatedUrl` 缓存。如果 Agent 用 `evaluate_script` 改 location 跳转 → Wraith 抓不到。**接受这个边界**，文档明示「敏感判定基于最近一次 navigate_page；evaluate_script 跳转无法识别」。
...
| `/browser connect` 是否改用户 mcp.json 文件 | **不改**，只在内存切；重启 Wraith 回 isolated |
