...
function installApi(initial: ActivitySnapshot, activityListImpl: () => Promise<ActivitySnapshot> = async () => initial) {
  ...
  // App 现在对后端事件有多个订阅者(活动刷新 / 会话异常标记等),真实 onEvent 基于
  // ipcRenderer.on 支持多监听器;mock 必须同语义,否则后注册的订阅会顶掉先注册的。
  const backendListeners = new Set<(event: BackendEvent) => void>()
  ...
  return {
    ...
    emitBackend: (value: BackendEvent) => backendListeners.forEach(listener => listener(value)),
  }
}
...
beforeEach(() => {
  localStorage.clear()
  // jsdom 无 ResizeObserver,WorkbenchTabBar 等组件挂载时依赖它做宽度检测
