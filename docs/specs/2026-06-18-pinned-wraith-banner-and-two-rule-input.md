...
```
   ...
   ╚███╔███╔╝██║  ██║██║  ██║██║   ██║   ██║  ██║     │
    ╚══╝╚══╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝   ╚═╝   ╚═╝  ╚═╝     │
    Wraith · v16.1.0 · DeepSeek-V4-Flash          │
   ────────────────────────────────────────────────  ┘
...
- **常驻顶部 banner**:WRAITH 字标 + 信息行(Wraith / Model / 状态 / 能力,`H=11`)经 DECSTBM 冻结在左上角,对话滚动时不被覆盖。组件:`render/inline/PinnedBanner.java`;滚动区接管 + 顶边纠正在 `BottomStatusBar`;去抖尺寸监视线程在 `InlineRenderer`。
