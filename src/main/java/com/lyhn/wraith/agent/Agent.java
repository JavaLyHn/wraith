...
import com.lyhn.wraith.memory.ExplicitMemoryHints;
import com.lyhn.wraith.memory.MemoryManager;
import com.lyhn.wraith.prompt.PromptAssembler;
import com.lyhn.wraith.prompt.PromptContext;
import com.lyhn.wraith.prompt.PromptMode;
import com.lyhn.wraith.prompt.ProjectMemoryLoader;
...
import com.lyhn.wraith.image.ImageReferenceParser;
...
public class Agent {
    ...
    private String buildProjectMemoryContext() {
        try {
            ...
            log.warn("Failed to load WRAITH.md project memory", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        com.lyhn.wraith.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.lyhn.wraith.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.lyhn.wraith.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.lyhn.wraith.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.lyhn.wraith.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.lyhn.wraith.memory.MemoryEntry.estimateTokens(
