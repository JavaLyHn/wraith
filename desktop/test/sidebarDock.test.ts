...
describe('dockInnerClass', () => {
  it('展开态:恒绝对定位 + translate-x-0(忽略 peek)', () => {
    ...
    for (const c of [c0, c1]) {
      ...
      expect(c).not.toContain('backdrop-blur-xl')
      expect(c).not.toContain('-translate-x-full')
    }
  })
  it('折叠+peek:绝对浮层(玻璃底防内容透叠)且 translate-x-0、无隐藏类', () => {
    const c = dockInnerClass(true, true)
    expect(c).toContain('absolute left-0 top-0')
    expect(c).toContain('z-50')
    expect(c).toContain('rounded-r-xl')
    expect(c).toContain('shadow-2xl')
    expect(c).toContain('bg-bg/70')
    expect(c).toContain('backdrop-blur-xl')
