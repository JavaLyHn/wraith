...
- `~/.wraith/mcp.json` 默认包含 `chrome-devtools` server，**默认 enabled**
...
Wraith CLI 启动时如果 `~/.wraith/mcp.json` 不存在，**自动创建**含 chrome-devtools 的最小模板。如果存在但缺 chrome-devtools 条目，启动时打印一行提示「检测到未配置 chrome-devtools，建议参考 README 添加」（不自动改用户文件）。
...
| 文件 | 改动 |
|---|---|
| `~/.wraith/mcp.json`（用户文件，启动时检测） | `Main.java` 启动时检测：文件不存在则创建默认含 chrome-devtools 的模板 |
| `src/main/java/com/lyhn/wraith/mcp/McpClient.java` | initialize 超时 30s → 60s（或读 `wraith.mcp.initialize.timeout.seconds` 系统属性） |
...
**产出**：
- `Main.java`：启动时检测 `~/.wraith/mcp.json`，不存在则用模板创建（含 chrome-devtools）
- `McpServerManager.startAll`：另起 status printer 线程；每 5s 打印未就绪 server 等待时长
- `McpClient.initialize`：超时从 30s 提到 60s，可被 `wraith.mcp.initialize.timeout.seconds` 系统属性覆盖
...
- [ ] §5 列出的所有现有文件都按要求修改
- [ ] `~/.wraith/mcp.json` 不存在时启动自动创建（含 chrome-devtools）
