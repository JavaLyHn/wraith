...
describe('toolBadgeLabel', () => {
  it('execute_command 显示真实退出码(成功/失败)', () => {
    expect(toolBadgeLabel(card({ name: 'execute_command', ok: true, exitCode: 0 }))).toBe('exit 0')
    expect(toolBadgeLabel(card({ name: 'execute_command', ok: false, exitCode: 127 }))).toBe('exit 127')
  })
  it('execute_command 缺 exitCode 时按 ok 兜底 0/1', () => {
    expect(toolBadgeLabel(card({ name: 'execute_command', ok: true }))).toBe('exit 0')
