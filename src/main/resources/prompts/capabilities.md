...
| **MCP** | 接外部 MCP server（stdio / HTTP），给自己加动态工具 | MCP 面板加 server（命令或 URL）→启用 / 重启；或编辑 `{{configDir}}/mcp.json`。`open_panel(plugins)` |
...
| **技能（Skills）** | 用户级 / 项目级 Skill 文件，按需 load | 技能面板新建 / 编辑 / 启用；或放 `{{configDir}}/skills`、`<项目>/.wraith/skills`。`open_panel(skills)` |
