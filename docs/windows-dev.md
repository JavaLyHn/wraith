...
> **v1.4.0 更新：这批已在真机上验过主链路。** 而且真机立刻证明了这段警告不是多余的 ——
> **管道 DACL 漏了创建者，AppContainer 从来就没真正起来过**（修复见 `51fe2d3`）。
> 也就是说在真机跑之前，这一整块的「代码完成」是**假绿**。
>
> 仍未逐条走完下面的清单，Win32 调用序列、icacls 授权、工具链可读性的边角情况
> 依然值得逐项过一遍。**任一条不过请把整段输出发出来。**
