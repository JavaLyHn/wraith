...
为什么当初必须从 `1.3.0` 改掉：`v1.3.0` 已经从 `main` 发过 mac 版。若不改，NSIS 会产出
`Wraith Setup 1.3.0.exe` —— 一个名字叫 1.3.0、内容完全不是 1.3.0 的安装包。

> **`-beta.1` 后缀已经去掉。** 它当初的理由是「这是第一个**从未经真机验证的分支**出的包」，
> 而 Windows 的主链路已经在真机上跑过（并在真机上揪出了 AppContainer 管道 DACL 那个缺陷，
> 见 `51fe2d3`），这个理由不再成立，所以 v1.4.0 是正式版、占 Latest。
>
> ⚠️ 但**没有逐条走完 `windows-dev.md` 的 124 条清单**——「主链路已验」不等于「全部验收通过」。
> 下一版发之前该把清单过完。
