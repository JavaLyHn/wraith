...
> 3. `src/main/java/com/lyhn/wraith/agent/Agent.java`、`PlanExecuteAgent.java`、`SubAgent.java`（系统提示词构造路径）
> 4. `src/main/java/com/lyhn/wraith/tool/ToolRegistry.java`（本期新增内置工具 `load_skill` 的注册位置）
> 5. `src/main/java/com/lyhn/wraith/cli/Main.java` / `CliCommandParser.java`（本期新增 `/skill` 命令组）
...
让 Wraith 拥有自己的 Skill 体系。Skill 是一个目录（含 `SKILL.md` + 可选 `references/` + 可选 `scripts/`），其中：
...
| 完整指令展开 | LLM 调 `load_skill("name")` 后，Wraith 把 SKILL.md 正文（去 frontmatter）拼到 next user message 的开头 |
...
- user message 是每轮重发，注入 body 会随轮次自然衰减（Wraith 控制只保留最近 1 轮的注入，避免堆积）
...
- **不直接返回正文给 LLM 当工具结果**，而是：
  ...
  - 把 body 写入 `SkillContextBuffer`（per-session，per-agent-role），下一轮 user message 末尾被 Wraith 拼接
...
**不做**：
- **不**复制 `~/.claude/skills/web-access/scripts/cdp-proxy.mjs`（那是 568 行的 HTTP 代理，Wraith 第 13 / 14 期已经有 chrome-devtools MCP + shared 模式，能力等价，重写一个反而引入维护负担）
...
| 文件 | 职责 |
|---|---|
| `src/main/java/com/lyhn/wraith/skill/Skill.java` | record，含 name / description / version / author / tags / source(builtin/user/project) / bodyPath / cachedReferencesDir |
| `src/main/java/com/lyhn/wraith/skill/SkillFrontmatterParser.java` | 极简 YAML 子集解析（§3.2） |
...
| 文件 | 改动 |
|---|---|
| `src/main/java/com/lyhn/wraith/cli/Main.java` | 启动期初始化 SkillBuiltinExtractor + SkillRegistry + SkillContextBuffer；Banner v15.0.0；启动 hint 加 `/skill list` 提示；`/skill` 命令分发 |
| `src/main/java/com/lyhn/wraith/cli/CliCommandParser.java` | 新增 `SKILL` 命令类型 + 子命令 `list` / `show <name>` / `on <name>` / `off <name>` / `reload` |
| `src/main/java/com/lyhn/wraith/tool/ToolRegistry.java` | 注册 `load_skill` 工具；构造时接收 SkillRegistry + SkillContextBuffer 引用 |
| `src/main/java/com/lyhn/wraith/agent/Agent.java` | system prompt 加 §3.8 skill 索引段；构造 user message 时调 `SkillContextBuffer.drain()` 拼接到原内容前 |
| `src/main/java/com/lyhn/wraith/agent/PlanExecuteAgent.java` | 同上 |
| `src/main/java/com/lyhn/wraith/agent/SubAgent.java` | 同上（三种角色都共享同一索引；buffer 在主 Agent 与子 Agent 间不共享，子 Agent 有自己的 buffer 实例） |
| `src/main/java/com/lyhn/wraith/agent/AgentOrchestrator.java` | 在创建 SubAgent 时为每个 SubAgent 构造独立 SkillContextBuffer |
...
📚 Skills（3 个）
  ...
  ● wraith-internal  user      v0.2.0  Wraith 项目内部约定与文档导航...
...
<SKILL.md body>
---
用户输入：（Wraith 不重复用户原话，但 LLM 已有上下文）
...
2. Wraith 输出 `已加载 skill 'web-access'...`
...
```
> 看下 Wraith 项目的 ROADMAP.md，再去网上搜一下 LangGraph4J 最新版本
```
...
清空 `~/.wraith/skills-cache/`，启动 Wraith：
...
4. **SkillContextBuffer 与流式输出竞态**：LLM 流式返回 tool_calls 时，Wraith 还没 flush 完上一轮 reasoning，就收到新一轮 user message——buffer drain 时机要在**新 user message 构造时**而不是上一轮 LLM 完成时，避免提前清空。
