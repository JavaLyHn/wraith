...
> Lanterna 三栏 TUI 作为可切换形态保留，通过 `WRAITH_RENDERER=lanterna` 启用。
...
> 4. `src/main/java/com/lyhn/wraith/cli/Main.java`（当前 CLI 入口，TUI 将在此之上做可视化层）
> 5. `src/main/java/com/lyhn/wraith/util/TerminalMarkdownRenderer.java`（现有 Markdown 渲染器，TUI 需复用或增强）
> 6. `src/main/java/com/lyhn/wraith/agent/Agent.java`、`PlanExecuteAgent.java`、`SubAgent.java`（系统提示词构造路径）
>
...
# ========== 第 16 期：TUI 产品化 ==========
# TUI 开关
# WRAITH_TUI=true       # 显式启用 Lanterna 全屏 TUI；默认保持 CLI
# NO_TUI=true           # 强制 CLI，覆盖 WRAITH_TUI=true
# WRAITH_THEME=dark     # TUI 主题: dark / light（默认 dark）

# TUI 窗口尺寸（Lanterna 自适应，无需手动设置）
# WRAITH_TUI_FILE_TREE_WIDTH=25%  # 文件树宽度（百分比或绝对列数）
# WRAITH_TUI_FONT_SIZE=1          # 字体缩放（0=小, 1=中, 2=大）
```
...
### 4.1 新增类（`com.lyhn.wraith.tui` 包）

| 文件 | 职责 |
|---|---|
| `src/main/java/com/lyhn/wraith/tui/TuiBootstrap.java` | TUI 入口，判断 `shouldUseTui()`，创建 `LanternaWindow` 和布局 |
| `src/main/java/com/lyhn/wraith/tui/LanternaWindow.java` | 封装 Lanterna `Window`，统一处理主题 / 大小变化 / 输入分发 |
| `src/main/java/com/lyhn/wraith/tui/pane/LeftPane.java` | 左侧文件树面板（`FileTreePane` 类，基于 Lanterna `TreeWidget` 或自定义 `ListWidget`） |
| `src/main/java/com/lyhn/wraith/tui/pane/CenterPane.java` | 中央对话流面板（`ChatPane`，基于 Lanterna `TextArea`，支持 Markdown 渲染 + 代码高亮 + 消息分色） |
| `src/main/java/com/lyhn/wraith/tui/pane/RightPane.java` | 右侧状态栏面板（`StatusPane`，显示模型 / Token / 耗时 / 命令提示） |
| `src/main/java/com/lyhn/wraith/tui/pane/InputBar.java` | 底部输入栏（`InputBar`，基于 Lanterna `TextField`，处理 Enter / Tab / ↑↓ / Ctrl 快捷键） |
...
| 文件 | 改动 |
|---|---|
| `src/main/java/com/lyhn/wraith/cli/Main.java` | TUI 入口判断（`shouldUseTui()`）；降级时保留现有 CLI；TUI 模式下用 `TuiBootstrap.launch()` 替换 JLine `LineReader` 主循环；Banner v16.0.0 |
| `src/main/java/com/lyhn/wraith/cli/CliCommandParser.java` | 新增 `CONFIG` 命令类型 + 子命令（`/config` 开配置面板）；TUI 模式下 `/clear` 清空对话流；`/exit` / `/quit` 关闭 TUI 窗口 |
| `src/main/java/com/lyhn/wraith/util/TerminalMarkdownRenderer.java` | 增强 Markdown 渲染器：支持代码块折叠 + 语法高亮输出（调用 `CodeHighlighter`）；保持 CLI 模式向后兼容 |
| `src/main/java/com/lyhn/wraith/config/WraithConfig.java` | 新增 `tui` 配置段（`fileTree` / `editor`）；新增 `ThemeManager` 读取 / 写入 `theme` 字段 |
| `src/main/java/com/lyhn/wraith/agent/Agent.java` | 流式输出时 TUI 模式改调用 `ChatPane.appendStreamingMessage()`（直接向面板写流式文本，不经过 `System.out`）；TUI 下的 HITL 弹窗调用 `TuiHitlDialog.show()` 替代 `TerminalHitlHandler` |
| `src/main/java/com/lyhn/wraith/hitl/TerminalHitlHandler.java` | 新增 `TuiHitlDialog` 内部类（Lanterna 模态框弹窗），保留 CLI 模式兼容；`requestApproval` 根据 `LanternaWindow.isTuiMode()` 分发到 TUI 或 CLI |
| `src/main/java/com/lyhn/wraith/agent/PlanExecuteAgent.java` | 同上：流式输出和 HITL 弹窗根据模式分发 |
| `src/main/java/com/lyhn/wraith/agent/SubAgent.java` | 同上：Multi-Agent 流式输出分发 |
...
**配置路径**：
1. `WraithConfig` 初始化时读 `config.json` 的 `tui.theme` 字段
2. `ThemeManager` 维护当前主题（`"dark"` 默认）
3. `/config` 编辑主题后调用 `WraithConfig.save(config)` 写回
4. 下次启动时读取
...
3. 用 `↑` / `↓` 移动选中 `Agent.java`，按 `Enter` 插入 `@file://src/main/java/com/lyhn/wraith/agent/Agent.java`
4. Agent 收到引用后调 `read_file` 并展示内容

### 7.3 代码高亮

```
> 帮我看下 src/main/java/com/lyhn/wraith/agent/Agent.java 里的 run() 方法做了什么
```
**期望**：
...
10. **降级 CLI 模式的 TUI 专属功能降级行为**：`/config` 在 CLI 模式下应该降级为打印配置值（`WraithConfig.toString()`），不弹出配置面板（因为 CLI 没有面板能力）。
...
**测试**：
- `TuiBootstrapTest`：未显式设置 `WRAITH_TUI=true` → `shouldUseTui()` 返回 `false`
- `TuiBootstrapTest`：`WRAITH_TUI=true` 且 `NO_TUI=true` → `shouldUseTui()` 返回 `false`
...
```
> 帮我看下 src/main/java/com/lyhn/wraith/agent/Agent.java 里的 Agent 类主要做了什么
```
...
**期望行为序列**：
1. 未显式设置 `WRAITH_TUI=true` → `shouldUseTui()` 返回 `false`
2. 不弹 Lanterna 全屏窗口
